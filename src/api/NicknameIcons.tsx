...
export function _renderIcons(props: NicknameIconProps) {
    return Array.from(nicknameIcons)
        ...
        .filter(icon => {
            ...
            // Filter out empty fragments or elements that might cause the dark circle
            // @ts-ignore
            if (typeof icon === "object" && "type" in icon && (icon.type === Symbol.for("react.fragment") || icon.type === React.Fragment)) {
                // @ts-ignore
