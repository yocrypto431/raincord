...
export const DATA_DIR = process.env.RAINCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "RAINCORDData", suffix)
        : join(app.getPath("userData"), "..", "RAINCORD", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[RAINCORD] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
