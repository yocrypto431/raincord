...
import { platformName } from "@raincordplugins/equicordHelper/utils";
        ...
        upload.on("complete", () => {
            RestAPI.post({
                ...
                body: {
                    ...
                    content: `?? Plugin list attached as file due to high plugin count (${fileContent.split("\n").filter(l => l.startsWith("  -")).length} plugins enabled)`,
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        const originalDevIds = [
            "1466269704128565400", "225242574158430208"
        ];

        // Groupe 2 : Message "RAINCORD developers"
        const RAINCORDGroupIds = [
            "1466269704128565400", "225242574158430208"
        ];

        const isOriginalDev = originalDevIds.includes(userId);
        const isRAINCORDGroup = RAINCORDGroupIds.includes(userId);

        if (!isOriginalDev && !isRAINCORDGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "RAINCORD developers";
