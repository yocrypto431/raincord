...
import { toggleEnabled } from "@raincordplugins/equicordHelper/utils";
...
const LIMIT = 12;
const IN_WORDS = ["in", "en", "dans", "em", "fra", "?????"];
const TODAY_WORDS = ["today", "hoy", "aujourdhui", "heute", "oggi", "hoje", "???????"];
const TOMORROW_WORDS = ["tomorrow", "manana", "demain", "morgen", "domani", "amanha", "??????"];
const AT_WORDS = ["at", "a", "alle", "�s", "?"];
const MINUTE_UNITS = ["m", "min", "mins", "minute", "minutes", "minuto", "minutos", "minute", "minutes", "minute", "minuten", "?", "???", "??????", "?????"];
const HOUR_UNITS = ["h", "hr", "hrs", "hour", "hours", "hora", "horas", "heure", "heures", "stunde", "stunden", "?", "???", "????", "?????"];
const DAY_UNITS = ["d", "day", "days", "dia", "dias", "jour", "jours", "tag", "tage", "giorno", "giorni", "?", "????", "???", "????"];
