...
function resetKrispPipeline() {
    try {
        ...
        const MediaSettingsStore = W.find(W.filters.byProps("setNoiseSuppressionLevel", "getNoiseSuppression"), { isIndirect: true })
            ?? W.find(W.filters.byProps("setNoiseSuppression", "getNoiseSuppression"), { isIndirect: true });
    ...
    start() {
        ...
        const patchNative = () => {
            ...
            try {
                ...
                try {
                    Object.defineProperty(DN.nativeModules, "requireModule", { value: patchedRequire, configurable: true, writable: true });
                } catch {
                    console.warn("[FixKrisp] Cannot patch requireModule — property is non-writable/non-configurable");
                    return;
                }
            }
