...
const globNativesPlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-natives" }, async () => {
            const pluginDirs = ["plugins", "userplugins", "raincordplugins"];
            let code = "";
            let natives = "\n";
            let i = 0;
            /**
...
const buildConfigs = ([
    ...
    },
    {
        ...commonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/Vencord.ts")],
        outfile: "dist/RAINCORD/renderer.js",
        format: "iife",
        target: ["esnext"],
    ...
    },
    {
        ...nodeCommonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/preload.ts")],
        outfile: "dist/RAINCORD/preload.js",
        footer: { js: "//# sourceURL=file:///VencordPreload\n" + sourceMapFooter("preload") },
        sourcemap,
        define: {
            ...defines,
...
await Promise.all([
    ...
    writeFile("dist/RAINCORD/package.json", JSON.stringify({
        name: "RAINCORD",
        main: "main.js"
    }))
]);

// -- Obfuscation d�sactiv�e d�finitivement -------------------
/*
if (!watch && !IS_DEV) {
    await Promise.all([
        obfuscateDir("dist/desktop"),
        obfuscateDir("dist/RAINCORD"),
    ]);
} else {
    console.log("[build] Obfuscation ignor�e en mode dev/watch.");
}
*/
console.log("[build] Obfuscation d�sactiv�e (Code Source en clair).");

await Promise.all([
    createPackage("dist/desktop", "dist/desktop.asar"),
    createPackage("dist/RAINCORD", "dist/RAINCORD.asar"),
]);
