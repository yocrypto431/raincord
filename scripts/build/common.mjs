...
let _gitHash = "unknown";
try {
    _gitHash = process.env.RAINCORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
} catch {
    _gitHash = process.env.RAINCORD_HASH || "no-git";
}
export const gitHash = _gitHash;
...
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "userplugins", "raincordplugins", "raincordplugins/_api"];
            let code = "";
            let pluginsCode = "\n";
            let metaCode = "\n";
            let excludedCode = "\n";
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.RAINCORD_REMOTE;
            if (!remote) {
                try {
